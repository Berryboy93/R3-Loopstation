...
export function Header({ bpm, setBpm }: HeaderProps) {
  ...
  const adjustBpm = (delta: number) => setBpm(Math.max(40, Math.min(280, bpm + delta)));

  // SIGN OUT flash — store the timeout handle and clear it on unmount so
  // we never call setSignOutFlash(false) on an unmounted component.
  ...
  const handleSignOut = () => {
    setSignOutFlash(true);
    if (signOutTimerRef.current !== null) clearTimeout(signOutTimerRef.current);
    signOutTimerRef.current = setTimeout(() => {
      setSignOutFlash(false);
  ...
  return (
    <div
      ...
      style={{ background: '#0d0d0d', borderBottom: '1px solid rgba(183,255,0,0.18)' }}
    >
      {/* ── Logo + Transport ── */}
        ...
        <div className="flex flex-col gap-[5px]">
          ...
          <div className="flex justify-between items-center px-[2px]">
            ...
            <button
              ...
              style={{
                fontFamily: "'Share Tech Mono', monospace", fontSize: 8,
                padding: '1px 6px', borderRadius: 2, cursor: 'pointer', transition: 'all 0.12s',
          ...
          {vuLevels.map((lvl, i) => (
            <div key={i} className="flex flex-col justify-end w-[7px] gap-[1px]">
              {Array.from({ length: 10 }).map((_, j) => {
                ...
                const lc     = j < 2 ? '#FF3B3B' : j < 4 ? '#FF8C00' : '#39FF14';
                return (
                  <div key={j} style={{
                    flex: 1, borderRadius: 1,
                    background: active ? lc : '#1a1a1a',
                    boxShadow:  active ? `0 0 3px ${lc}80` : 'none',
        ...
        <div className="flex flex-col gap-[5px]">
          <div className="flex gap-1">
            <button
              ...
              style={{
                fontFamily: "'Share Tech Mono', monospace", fontSize: 9,
                padding: '3px 7px', borderRadius: 2, cursor: 'pointer', transition: 'all 0.12s',
          ...
          {/* SIGN OUT — timer stored in ref, cleared on unmount */}
          <button
